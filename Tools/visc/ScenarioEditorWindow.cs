...
namespace Visc
{
    public class ScenarioEditorWindow : EditorWindow
    {
        ...
        private EventAction _draggedAction, _selectedAction;
        ...
        private void DrawActions(List<EventAction> actions, float offset, float duration, float hOffset, int _trackOffset)
        {
            ...
            foreach (var action in actions)
            {
                ...
                action.DrawTimelineGui(boxRect, action == _selectedAction);
                    ...
                    (Event.current.type == EventType.MouseDown || Event.current.type == EventType.ContextClick) && (startHandle || mainHandle || endHandle))
                {
                    if (Event.current.button == 0)
                    {
                        ...
                        {
                            case 1:
                                _draggedAction = _selectedAction = action;
                                _startPositionX = Event.current.mousePosition.x;
                    ...
                    else if (Event.current.button == 1 || Event.current.type == EventType.ContextClick)
                    {
                        ...
                        actionMenu.AddItem(new GUIContent("Duplicate"), false, DuplicateContextItem, action);
        ...
        private void DuplicateContextItem(object obj)
        {
            var action = obj as EventAction;
            if (action == null) return;

            var copy = ScriptableObject.CreateInstance(action.GetType()) as EventAction;
            EditorUtility.CopySerialized(action, copy);

            copy.StartTime = action.StartTime + action.Duration;
            CurrentScenario.DuplicateAction(copy);
