...
public class SimplestPlot : MonoBehaviour
{
    ...
    public void SetResolution(Vector2 NewResolution)
    {
        ...
        StartPosX = TextureResolution.x * 0.1f;
        StartPosY = TextureResolution.y * 0.1f;
    }
    ...
    public void UpdatePlot()
    {
        ...
        if (MyPlotType == PlotType.TimeSeries)
        {
            ...
            ScaleX = (float)(PlotTexture.width - 2 * StartPosX) / (float)(MinMaxOfPlotsX[max] - MinMaxOfPlotsX[min]);
            ScaleY = (float)(PlotTexture.height - 2 * StartPosY) / (float)(MinMaxOfPlotsY[max] - MinMaxOfPlotsY[min]);

            SetupAxes(ScaleY, MinMaxOfPlotsY[min], MinMaxOfPlotsY[max]);

            for (int Cnt = 0; Cnt < SeriesPlotY.Count; Cnt++)
                DrawTimeSeriesPlot(ScaleX, ScaleY, SeriesPlotX, MinMaxOfPlotsX[min], SeriesPlotY[Cnt].YValues, MinMaxOfPlotsY[min], SeriesPlotY[Cnt].MyColor, Cnt * 5);
        }
    ...
    // Draw Functions
    private void DrawTimeSeriesPlot(float PlotScaleX, float PlotScaleY, float[] XPoints, float MinX, float[] YPoints, float MinY, Color LineColor, int offset = 0)
    {
        ...
        for (int Cnt = 1; Cnt < YPoints.Length; Cnt++)
        {
            ...
            DrawLine(PlotTexture, FirstX + offset, FirstY, SecondX + offset, SecondY, LineColor);
            DrawFilledCircle(PlotTexture, FirstX + offset, FirstY, DataPointRadius, LineColor);

            //draw last datapoint
            if(Cnt == YPoints.Length - 1)
                DrawFilledCircle(PlotTexture, SecondX + offset, SecondY, DataPointRadius, LineColor);
        }
    }
    ...
    void DrawFilledCircle(Texture2D PlotTexture, int x0, int y0, int radius, Color CircleColor)
    {
        int x = radius;
        int y = 0;
        ...
        while (x >= y)
        {
            ...
            {
                PlotTexture.SetPixel(i, y0 + y, CircleColor);
                PlotTexture.SetPixel(i, y0 - y, CircleColor);
            }
            for (int i = x0 - y; i <= x0 + y; i++)
            {
                PlotTexture.SetPixel(i, y0 + x, CircleColor);
                PlotTexture.SetPixel(i, y0 - x, CircleColor);
            }
    ...
    private void UpdateText(float[] MinMaxOfPlotsX, float[] MinMaxOfPlotsY)
    {
        ...
        if (AxesVisible && MinMaxOfPlotsY[1] < float.MaxValue && MinMaxOfPlotsY[0] > float.MinValue)
        {
            ...
            {
                PlotXMaxText.text = MinMaxOfPlotsX[1].ToString("0");
                PlotXMinText.text = MinMaxOfPlotsX[0].ToString("0");
            }
        }
        else
        {
    ...
    private void SetupAxes(float scaleY, float minY, float maxY)
    {
        ...
        float startY;
        // check if minY is an integer (ie x.00 and not x.53)
        if (minY != Mathf.Ceil(minY))
            startY = StartPosY - (minY - Mathf.Ceil(minY)) * scaleY;
        else
            startY = StartPosY;

        // y lines
        for(float i = startY; i <= ((maxY-minY)*scaleY); i += scaleY)
        {
            DrawLine(PlotTexture, padding, Mathf.RoundToInt(i), PlotTexture.width - padding, Mathf.RoundToInt(i), new Color(.8f,.8f,.8f));
        }

        // main X
        DrawLine(PlotTexture, (int)(StartPosX * .5f), padding, (int)(StartPosX * .5f), PlotTexture.height - padding, Color.black);
        DrawLine(PlotTexture, (int)(StartPosX * .5f) + 1, padding, (int)(StartPosX * .5f)+1, PlotTexture.height - padding, Color.black);

        // main Y
        DrawLine(PlotTexture, padding, (int)(StartPosY * .5f), PlotTexture.width - padding, (int)(StartPosY * .5f), Color.black);
        DrawLine(PlotTexture, padding, (int)(StartPosY * .5f) + 1, PlotTexture.width - padding, (int)(StartPosY * .5f) + 1, Color.black);
    }
    ...
    private void SetupText()
    {
        ...
        PlotYMinText.alignment = TextAnchor.LowerCenter;
        PlotYMaxText.alignment = TextAnchor.LowerCenter;

        PlotXMinText.alignment = TextAnchor.MiddleCenter;
        PlotXMaxText.alignment = TextAnchor.MiddleCenter;
        ...
        RectTransform rect;
        // Y Min
        rect = PlotYMinText.GetComponent<RectTransform>();
        ...
        // Y Max
        rect = PlotYMaxText.GetComponent<RectTransform>();
        rect.anchorMin = new Vector2(0, 0.9f);
        rect.anchorMax = new Vector2(0.05f, 1);
        rect.offsetMin = new Vector2(0, 0);
        rect.offsetMax = new Vector2(0, 0);

        // X min
        rect = PlotXMinText.GetComponent<RectTransform>();
        rect.anchorMin = new Vector2(0.08f, 0);
        rect.anchorMax = new Vector2(0.13f, 0.05f);
        rect.offsetMin = new Vector2(0, 0);
        rect.offsetMax = new Vector2(0, 0);

        // X max
        rect = PlotXMaxText.GetComponent<RectTransform>();
        rect.anchorMin = new Vector2(0.87f, 0);
        rect.anchorMax = new Vector2(.93f, 0.05f);
        rect.offsetMin = new Vector2(0, 0);
        rect.offsetMax = new Vector2(0, 0);

        // Warning
        rect = WarningText.GetComponent<RectTransform>();
